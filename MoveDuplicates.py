...
def iterate_paths():
  ...
  if mp_shapes:
    logs(f"Spawning {len(mp_shapes)} subprocesses for the following shapes :")
    for image_batch, ext, shape, images_nb in mp_shapes:
      logs(f" • {ext} {shape} -> {images_nb} elements to scan")
